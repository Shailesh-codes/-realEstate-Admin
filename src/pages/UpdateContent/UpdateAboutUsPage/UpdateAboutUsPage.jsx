...
function UpdateAboutUsPage() {
  ...
  const handleUpdateAbout = async () => {
    try {
      ...
      const response = await axios.put(`${api}/about/update`, formData, {
        headers: {
  ...
  useEffect(() => {
    const fetchAboutContent = async () => {
      try {
        ...
        if (response.data.data?.about?.description) {
          setAbout(response.data.data.about.description);
        }
      } catch (error) {
      ...
      <div className="px-4 lg:px-7 py-4 lg:py-8 bg-white rounded-tl-xl flex flex-col items-center justify-center gap-4 lg:gap-7">
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <div className="w-full flex justify-center mt-6">
            <button
              className="border border-[#0b2c3d] bg-[#af0808] hover:bg-[#0b2c3d] text-[white] rounded-sm transition-colors py-3 px-6 lg:px-10 text-sm lg:text-base"
